...
const useStyles = makeStyles({
  page:          { display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalL, padding: tokens.spacingVerticalL, maxWidth: '960px', margin: '0 auto' },
...
const TYPE_COLORS: Record<string, string> = {
  Confirm: '#107C10', FalsePositive: '#D83B01', WrongOwner: '#8764B8', Duplicate: '#0078D4',
};
...
export function AdminDashboard({ authToken }: AdminDashboardProps): JSX.Element {
  ...
  const maxBreakdownVal = (obj: Record<string, number>) => Math.max(1, ...Object.values(obj));

  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div>
          <Text size={600} weight="bold">Commit Admin</Text>
        ...
        {tab === 'overview' && (
          <Button appearance="secondary" disabled={loadingMetrics} onClick={() => void loadMetrics()}
            icon={loadingMetrics ? <Spinner size="tiny" /> : undefined}>
            {loadingMetrics ? 'Refreshing…' : '↻ Refresh'}
          </Button>
        )}
        {tab === 'feedback' && (
          <Button appearance="secondary" disabled={fbLoading} onClick={() => void loadFeedback()}
            icon={fbLoading ? <Spinner size="tiny" /> : undefined}>
            {fbLoading ? 'Loading…' : '↻ Refresh'}
          </Button>
        )}
        {tab === 'pipeline' && (
          <Button appearance="secondary" disabled={profLoading} onClick={() => void loadProfiles()}
          ...
          <Card>
            <CardHeader header={
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                ...
                <Button appearance="subtle" size="small" disabled={loadingInsights} onClick={() => void loadInsights()}
        ...
        <>
          <Card>
            <div className={styles.filterBar}>
              ...
              <Button size="small" appearance="primary" onClick={() => void loadFeedback()} disabled={fbLoading}>
                  ...
                  <tbody>
                    {feedbackData.items.map((item, i) => (
                      <tr key={i}>
                        ...
                        <td className={styles.td}>
                          <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '12px', backgroundColor: (TYPE_COLORS[item.type] ?? '#888') + '22', color: TYPE_COLORS[item.type] ?? '#888', fontSize: '11px', fontWeight: 600 }}>
                            {item.type === 'FalsePositive' ? '👎 FalsePositive' : item.type === 'Confirm' ? '👍 Confirm' : item.type}
                          </span>
                        </td>
                        <td className={styles.td}>
